...
class YUVFrame{
public:
    YUVFrame(const YUVFrame& frame) {
        this->width = frame.width;
        this->height = frame.height;
        this->Y_data = frame.Y_data;
        this->Cb_data = frame.Cb_data;
        this->Cr_data = frame.Cr_data;
    }
    ...
    void copy_from_frame(YUVFrame& target) {
        std::copy(target.Y_data.begin(), target.Y_data.end(), Y_data.begin());
        std::copy(target.Cb_data.begin(), target.Cb_data.end(), Cb_data.begin());
        std::copy(target.Cr_data.begin(), target.Cr_data.end(), Cr_data.begin());
    }
    ...
    template <typename T>
    void set_Y_data(const std::vector<std::vector<T>>& in, YUVFrame &ref) {
        for (unsigned int y = 0; y < height; y++) {
            for (unsigned int x = 0; x < width; x++) {
                Y(x, y) = round_and_clamp_to_char(in.at(y).at(x) + ref.Y(x, y));
            }
        }
    }

    template <typename T>
    void set_Cb_data(const std::vector<std::vector<T>>& in, YUVFrame &ref) {
        for (unsigned int y = 0; y < height/chroma_ratio_y; y++) {
            for (unsigned int x = 0; x < width/chroma_ratio_x; x++) {
                Cb(x, y) = round_and_clamp_to_char(in.at(y).at(x) + ref.Cb(x, y));
            }
        }
    }
    
    template <typename T>
    void set_Cr_data(const std::vector<std::vector<T>>& in, YUVFrame &ref) {
        for (unsigned int y = 0; y < height/chroma_ratio_y; y++) {
            for (unsigned int x = 0; x < width/chroma_ratio_x; x++) {
                Cr(x, y) = round_and_clamp_to_char(in.at(y).at(x) + ref.Cr(x, y));
            }
        }
    }
    void set_Y_data(const std::vector<std::vector<unsigned char>>& in) {
        for (unsigned int y = 0; y < height; y++) {
            for (unsigned int x = 0; x < width; x++) {
                Y(x, y) = in.at(y).at(x);
            }
    ...
    void set_Cb_data(const std::vector<std::vector<unsigned char>>& in) {
        for (unsigned int y = 0; y < height/chroma_ratio_y; y++) {
            for (unsigned int x = 0; x < width/chroma_ratio_x; x++) {
                Cb(x, y) = in.at(y).at(x);
            }
    ...
    void set_Cr_data(const std::vector<std::vector<unsigned char>>& in) {
        for (unsigned int y = 0; y < height/chroma_ratio_y; y++) {
            for (unsigned int x = 0; x < width/chroma_ratio_x; x++) {
                Cr(x, y) = in.at(y).at(x);
            }
